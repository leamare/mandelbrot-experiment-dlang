/**
 * High-precision floating-point types using MPFR library.
 */
module calc.types.mpfr;
...
struct GMPComplex {
    ...
    void powFractionalAndAdd(R)(double n, auto ref R c) if (isGMPComplex!R) {
        import deimos.mpfr : mpfr_sqrt, mpfr_pow, mpfr_atan2, mpfr_sin, mpfr_cos, mpfr_set;
        ...
        auto re2 = sqr(re);
        auto im2 = sqr(im);
        auto magSquared = re2 + im2;
        
        auto mag = GMPFloat(0.0);
        mpfr_sqrt(mag.value.mpfr, magSquared.value.mpfr, mpfr_rnd_t.MPFR_RNDN);
        
        auto theta = GMPFloat(0.0);
        auto imCopy = GMPFloat(im);
        auto reCopy = GMPFloat(re);
        mpfr_atan2(theta.value.mpfr, imCopy.value.mpfr, reCopy.value.mpfr, mpfr_rnd_t.MPFR_RNDN);
        
        auto expN = GMPFloat(n);
        auto magPowN = GMPFloat(0.0);
        mpfr_pow(magPowN.value.mpfr, mag.value.mpfr, expN.value.mpfr, mpfr_rnd_t.MPFR_RNDN);
        
        auto nTheta = theta * expN;
        
        auto cosNTheta = GMPFloat(0.0);
        auto sinNTheta = GMPFloat(0.0);
        mpfr_cos(cosNTheta.value.mpfr, nTheta.value.mpfr, mpfr_rnd_t.MPFR_RNDN);
        mpfr_sin(sinNTheta.value.mpfr, nTheta.value.mpfr, mpfr_rnd_t.MPFR_RNDN);
        
        auto newRe = magPowN * cosNTheta;
        auto newIm = magPowN * sinNTheta;
        
        re = newRe + c.re;
        im = newIm + c.im;
    }
    
    GMPComplex powFractional(double n) const {
        import deimos.mpfr : mpfr_sqrt, mpfr_pow, mpfr_atan2, mpfr_sin, mpfr_cos, mpfr_set;
...
/**
 * Pixel-to-complex coordinate converter
 */
